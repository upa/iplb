 ...
 *
 * XXX
 * PATRICIA_MAXBITS is changed to 64 from 128,
 * becase size of array for 128 pointers is 1024 bytes in 64bit env.
 * It causes -Wframe-larger-than warning.
 */
...
#include <linux/types.h> /* for u_* definitions (on FreeBSD 5) */

#include <linux/errno.h> /* for EAFNOSUPPORT */
#ifndef EAFNOSUPPORT
#  defined EAFNOSUPPORT WSAEAFNOSUPPORT
#  include <winsock.h>
#else
#  include <uapi/linux/in.h> /* for struct in_addr */
#  include <uapi/linux/in6.h> /* for struct in_addr */
#endif
...
typedef void (*void_fn_walk_t)(prefix_t *, void *);
typedef void (*void_fn_walk2_t)(prefix_t *, void *, void *, void *);
...
void patricia_process (patricia_tree_t *patricia, void_fn_walk_t func);
void patricia_process2 (patricia_tree_t *patricia, void_fn_walk2_t func,
...
#define PATRICIA_MAXBITS        64
#define PATRICIA_NBIT(x)        (0x80 >> ((x) & 0x7f))
#define PATRICIA_NBYTE(x)       ((x) >> 3)

// XXX
// MAXBITS 128 causes large frame size (>2014). 
// IPv6 routes must be shorter than 65... (for kernel use)
